...
TEST_PROPERTY("`swap_first` swaps the first elements of two parts", [](vector_partitioning<int> vp) {
  RC_PRE(vp.partitioning_.parts_count() >= 2);
...
TEST_PROPERTY("`swap_first` can be used multiple times without error", [](vector_partitioning<int> vp) {
  RC_PRE(vp.partitioning_.parts_count() >= 2);
...
TEST_PROPERTY("`swap_first` on the same part should be a no-op", [](vector_partitioning<int> vp) {
  RC_PRE(vp.partitioning_.parts_count() >= 1);
...
TEST_PROPERTY("`swap_first` twice returns to original state (idempotent)", [](vector_partitioning<int> vp) {
  RC_PRE(vp.partitioning_.parts_count() >= 2);
...
TEST_PROPERTY("`swap_first` only modifies first elements of the two parts", [](vector_partitioning<int> vp) {
  RC_PRE(vp.partitioning_.parts_count() >= 2);
